...
export const setupLighting = (scene, paintings) => {
  const debugGuiEnabled = new URLSearchParams(window.location.search).has(
    "debug"
  );
  const gui = debugGuiEnabled ? new GUI() : null;
  ...
  ); // Spotlight for the statue — angled so it rakes across and reveals the carving
  statueSpotlight.intensity = 1.6;
  statueSpotlight.angle = 0.55;

import { handleMouseLook, keysPressed } from "./movement.js"; // import the keysPressed object
import { showMenu, hideMenu } from "./menu.js"; // import the showMenu function
import { startAudio, stopAudio } from "./audioGuide.js";
import { clearPaintingFocus } from "./paintingFocus.js";
...
export const setupEventListeners = (controls, camera, scene) => {
  ...
  document.addEventListener(
    "mousemove",
    (event) => handleMouseLook(event, controls),
    false
  );
...
function onKeyDown(event, controls) {
  ...
  if (event.key in keysPressed) {
    ...
    clearPaintingFocus(); // moving (WASD) steps back out of a zoomed artwork
  }

  if (event.key === "Escape") {
    clearPaintingFocus(); // ESC also exits the zoomed POV
